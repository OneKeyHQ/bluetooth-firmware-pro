...
static uint8_t axp_device_addr;

void axp216_set_i2c_addr(uint8_t addr) { axp_device_addr = addr; }
...
ret_code_t axp216_write(const uint8_t writeAddr, const uint8_t writeData) {
  ...
  ret = nrf_drv_twi_tx(&axp216_m_twi, axp_device_addr, tx_buff, AXP_ADDRESS_LEN + 1, false);
...
ret_code_t axp216_read(uint8_t readAddr, uint8_t byteNum, uint8_t* readData) {
  ...
  do {
    while (nrf_drv_twi_is_busy(&axp216_m_twi))
      ;
    ret = nrf_drv_twi_tx(&axp216_m_twi, axp_device_addr, &readAddr, AXP_ADDRESS_LEN, false);
    if (NRF_SUCCESS != ret) {
      break;
    }
    ret = nrf_drv_twi_rx(&axp216_m_twi, axp_device_addr, readData, byteNum);
    if (NRF_SUCCESS != ret) {
      break;
    }
  } while (0);
