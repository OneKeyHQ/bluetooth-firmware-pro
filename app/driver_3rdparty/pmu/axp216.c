...
Power_Error_t axp216_init(void) {
  do {
    // interface init()
    if (!pmu_interface_p->Init()) break;

    // get id
    uint8_t val = 0;
    if (!axp216_reg_read(AXP216_IC_TYPE, &val)) break;
...
Power_Error_t axp216_deinit(void) {
  do {
    // interface init()
    if (!pmu_interface_p->Deinit()) break;
...
Power_Error_t axp216_get_status(Power_Status_t* status) {
  ...
  status->chargerAvailable = (((tmp & (1 << 7) & (1 << 6)) == ((1 << 7) & (1 << 6))) ||  // acin
