...
void set_send_stm_data_p(void (*send_stm_data_p_)(uint8_t* pdata, uint8_t lenth)) { send_stm_data_p = send_stm_data_p_; }
...
static bool pmu_if_init() {
  ...
  if (!pwr_i2c_configured) {
    const nrf_drv_twi_config_t twi_config = {.scl = TWI_SCL_M, .sda = TWI_SDA_M, .frequency = NRF_DRV_TWI_FREQ_400K, .interrupt_priority = APP_IRQ_PRIORITY_HIGH, .clear_bus_init = true};
...
static bool pmu_if_send(const uint8_t device_addr, const uint32_t len, const uint8_t* const data) {
  ...
  // check i2c bus
  if (!pwr_i2c_configured) return false;
...
static bool pmu_if_reg_write(const uint8_t device_addr, const uint8_t reg_addr, const uint8_t val) {
  ...
  if (!pmu_if_send(device_addr, sizeof(tx_buff), tx_buff)) return false;
...
static bool pmu_if_reg_read(const uint8_t device_addr, const uint8_t reg_addr, uint8_t* const val) {
  ...
  if (!pmu_if_send(device_addr, sizeof(tmp), &tmp)) return false;
  if (!pmu_if_receive(device_addr, sizeof(*val), val)) return false;
...
static bool pmu_if_reg_set_bits(const uint8_t device_addr, const uint8_t reg_addr, const uint8_t bit_mask) {
  ...
  do {
    if (!pmu_if_reg_read(device_addr, reg_addr, &reg_val)) break;

    if ((reg_val & bit_mask) != bit_mask) {
      reg_val |= bit_mask;
      if (!pmu_if_reg_write(device_addr, reg_addr, reg_val)) break;
      ;
    }
...
bool power_manage_init() {
  ...
  if (pmu_p == NULL) return false;

  // init
  if (!pmu_p->isInitialized) pmu_p->Init();
...
bool power_manage_deinit() {
  ...
  // deinit
  if (pmu_p->isInitialized) pmu_p->Deinit();
    ...
    case POWER_IC_OK_IO:
      ...
      if (action == NRF_GPIOTE_POLARITY_TOGGLE) {
        ...
        if (nrf_gpio_pin_read(pin)) {
          ...
          if (i2c_configured) i2c_control(false);
        }
